...
<!-- 學習 `DPDK` 似乎可以順便掌握網路協議相關知識 `(L2-L4)` 以及各種軟硬體優化技巧 (如 `ring buffer`)，覺得是非常一石二鳥的好事。 -->
...
- `rte_ring`
    ...
    - `enqueue` 和 `dequeue` 的行為:

        - `prod` 和 `cons` 都各自有 `head` 和 `tail`，`head` 用來記錄使用權，`tail` 用來告訴其他 thread 完成到哪 (publish)
        - 分成 SP/SC 和 MP/MC，其中 MP/MC 又有三種不同的同步方式 classic/RTS/HTS，各自對 `tail` 更新的邏輯有所差異，也對 latency 和 throughput 有不同的影響

            - SP/SC 因為不用擔心同步問題，會直接更新 `head` 和 `tail`
            - Classic MP/MC 會用 `CAS` 搶奪空間/資源使用權，進行使用完畢後再 `spin` 等到前面的人都更新 `tail`，自己再更新
            - RTS 是 Relaxed Tail Sync，我還沒有看得很懂(TODO)，但大致上就是一樣用 CAS 拿使用權，然後檢查是不是大家都好了一次更新 tail，否則留下紀錄直接離開。
            - HTS 是 Head-Tail Sync，同一時間只讓一個搶到使用權的 thread 進行操作，其他人 `spin` 等待。

        - Classic 細節: enqueue/dequeue 時 3 個階段: 
            - 先修改 `head` 佔據空間
            - 確保空間/資源之後將資料複製到目標位置上 (也可能是複製指標，來減少複製成本)
            - 持續 `rte_pause()` 等待直到 `tail` 變成 `head` **修改前**的值，再將 `tail` 修改成  `head` 的新值

        - enqueue 和 dequeue 是對稱的，enqueue 看的是 cons.tail 來決定是否有足夠的空間來放，dequeue 看的是 prod.tail 來決定是否有足夠的資料來取，所以要完成修改 tail 的操作才算 enqueue/dequeue 完成，反之亦然。之所以要等待 `tail` 變成 `head` 修改前的值，是因為可能會有多個 threads 同時搶，先搶到的 threads 必須先完成才能開放後面的 threads 修改 tail，否則可能會取到未完成操作的空間/物件。
        
        - 沒搞懂的地方:
            - 下面這段程式碼 `lib/ring/rte_ring_elem_pvt.h` 為什麼會用 `&r[1]`，這樣到底是什麼意思?
            ```c
...
- `rte_mempool`
    ...
    - `rte_mempool` 的設計很有彈性，可以讓用戶在建立的時候決定要用哪一種底層實作，可以是依賴 `rte_ring` 的預設實作，也可以採用其他的，所有選項都放在 `drivers/mempool` 裡面 (`lib/mempool/rte_mempool.h` 的 `struct rte_mempool_ops`)，以下若需要指定實作皆以 `rte_ring` 為例。

    - `rte_mempool_get` --> `rte_mempool_get_bulk` 是空間的 cons，會對應到 ring 的 dequeue 操作
    - `rte_mempool_put` --> `rte_mempool_put_bulk` 是空間的 prod，會對應到 ring 的 enqueue 操作
