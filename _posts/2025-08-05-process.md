...
檔案系統中有個目錄 `/proc/<pid>` 專門用來輸出各種和該 `pid` 的 `process` 相關的資訊，會把很多 `task_struct` 內的資訊經過整理放在這裡，透過這個目錄底下的資訊，我們就能很大程度了解這個 `process`，最好的是這個方法幾乎不需要任何困難技術就可以觀察到 `task_struct`，我想要以此目錄展開各項關於 process 和 thread 的細節。
...
這段程式碼給了我們一個方便的方式去決定 `/proc/<pid>` 裡面有哪些項目，並且各自是什麼類型﹑接受什麼操作，具體的實作寫在同一個檔案 `linux/fs/proc/base.h` 的這個區塊
