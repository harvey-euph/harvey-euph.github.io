---
title: "Linux: Process"
description: 一些關於 Linux Process 的隨手記，想到什麼就整理什麼
...
其實 `task_struct` 代表的只是 `thread`，而 `process` 其實是用 `thread group` 的概念，將多個 `thread` 的 `task_struct` 使用共同的一些成員來表示這些 `task_struct` 是屬於同一個 `process` 的

每個 `thread` 會擁有一個只屬於自己獨一無二的 `pid`，並且屬於同一個 `thread group` 的 `thread` 會共用同一個 `tgid`，通常也會共用 `fs` 和 `mm`

每個 `thread group` 會有一個 `main thread`，他的 `pid = tgid`
...
檔案系統中有個目錄 `/proc/<pid>` 專門用來輸出各種和該 `pid` 對應的 `process` 相關的資訊，透過這個目錄底下的資訊，我們就能很大程度了解這個 `process`，最好的是這個方法幾乎不需要任何困難技術就可以觀察到 `task_struct`，我想要以此目錄展開各項關於 process 和 thread 的細節。

要記得 `sudo su` 來使用 `root` 權限，很多都是 `root` 才能操作的[^fn1]
...
這些大部分的內容其實不是真的存在於硬碟的檔案，是當我們 access 的時候由 kernel 動態生成的虛擬檔案系統 `(Virtual Filesystem, VFS)`

這些項目是由 `linux/fs/proc/base.c` 控制的，
...
可以看到上面定義的 `DIR("task", /* ... */)` 就表示 `/proc/<pid>/` 底下會有一個目錄叫做 `/proc/<pid>/task/`，進去一看會發現他是一些數字，那些就是這個 `process` 底下的 `threads` 的 `tid`，如果再走進去一看會發現 `/proc/<pid>/task/<tid>/` 的成員和 `/proc/<pid>/` 非常相似，但其是是稍微不同的，定義在底下這個區塊
...
可以很明顯發現這裡比上面少了 `DIR("task", /* ... */)` 的目錄

接下來我們來看看這些成員被定義的方式，位於同一個檔案
...
這段給了我們決定有哪些項目﹑各自是什麼類型﹑接受什麼操作，具體的實作寫在同一個檔案 `linux/fs/proc/base.h`
