...
>`getpid()` 會回傳 `tgid`<br>`gettid()` 會回傳 `pid`
{: .prompt-info }
...
接下來我會整理 `/proc/<pid>` 裡面的各項目的意義﹑生成的函數 (來自 `fs/proc/base.c`)﹑以及對應 `task_struct` 的來源


### 符號連結

在 linux 中有些檔案代表的是一個指向其他路徑的符號連結（symlink），對這些項目使用 `readlink` 可以看到他們指向的目標，`/proc/<pid>/` 底下也有幾個
...
| Name         | Access Function         | Target               | Meaning                                     |
| :----------- | :---------------------- | :------------------- | :------------------------------------------ |
